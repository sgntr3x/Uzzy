...
if HAS_GEMINI:
    try:
        ...
        if GEMINI_API_KEY:
            ...
            print("Uyarı: 'GEMINI_API_KEY' ortam değişkeni bulunamadı. AI özelliği devre dışı bırakıldı.")
            HAS_GEMINI = False
...
class UzzyGUI:
    def __init__(self, root):
        ...
        # Switch şasisi efekti için karanlık çerçeve
        ...
        # Paneli başlangıç değeri olan 48'e göre çiz
        ...
        # Koyu temalı özel Scrollbar stili oluştur (Windows'ta clam teması override yapmaya izin verir)
        style = ttk.Style()
        if "clam" in style.theme_names():
            style.theme_use("clam")
            
        # 3D efektleri (lightcolor/darkcolor) ve tırtıkları (gripcount) kapatarak tam flat/modern bir görünüm elde et
        ...
        # Font boyut ayarları (Sağa hizalı)
    ...
    def send_shortcut_command(self, cmd):
        """Kısayol butonlarından gelen komutları terminale gönderir."""
    ...
    def draw_port_panel(self):
        """Seçilen port sayısına göre fiziksel switch görünümünü yeniden çizer."""
        # Mevcut paneli (butonları) temizle
    ...
    # --- PORT SEÇİM LOKALİZASYONU VE SÜRÜKLE BIRAK MANTIĞI ---
    ...
    # --- POPUP MENÜ VE İŞLEMLER ---
    
    def open_ai_config_popup(self):
        """AI tabanlı yapılandırma için popup penceresini açar."""
        if not HAS_GEMINI:
            messagebox.showerror("Eksik Kütüphane", "AI özelliğini kullanmak için 'google-generativeai' kütüphanesi yüklü olmalıdır.\nLütfen terminalde 'pip install google-generativeai' komutunu çalıştırın.")
            return
        ...
        def on_apply():
            ...
            # AI işlemini arka planda çalıştır
    ...
    def _apply_ai_commands(self, commands_text, popup):
        """AI tarafından oluşturulan komutları kullanıcı onayıyla uygular."""
        ...
        # Kullanıcıya onay için komutları göster
    ...
    def open_create_vlan_popup(self):
        ...
        def apply_create():
            ...
            # IP veya Mask'tan biri doldurulduysa ikisi de zorunludur ve doğru formatta olmalıdır
            if ip or mask:
            ...
            # Kullanıcının arka arkaya yeni VLAN ekleyebilmesi için form alanlarını temizle
    ...
    def open_management_ip_popup(self):
        ...
        def dec_port():
            ...
            if val.isdigit() and int(val) > 1:
                ...
                selected_port.set("")  # 1'den aşağı inilirse veya hatalı metin varsa temizle
        ...
        def inc_port():
            ...
            if val.isdigit():
                ...
                selected_port.set("1")  # Boşsa veya hatalı metin varsa 1'den başlat
    ...
    def refresh_connection(self):
        """Mevcut seri port bağlantısını koparıp yeniden tarama başlatır."""
